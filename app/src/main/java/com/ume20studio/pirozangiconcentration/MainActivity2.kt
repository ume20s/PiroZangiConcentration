...
class MainActivity2 : AppCompatActivity() {
    ...
    val cardnum: Int = 8                // カード枚数
    val basepoint = 20                  // 基本ポイント
    var score: Int = 0                  // スコア
    ...
    // カードのイメージボタン配列
    val bt = arrayOf(R.id.ImgBtn2_01, R.id.ImgBtn2_02, R.id.ImgBtn2_03, R.id.ImgBtn2_04,
        R.id.ImgBtn2_05, R.id.ImgBtn2_06, R.id.ImgBtn2_07, R.id.ImgBtn2_08)
    ...
    // ミスインジケータのグラフィック配列
    val peke = arrayOf(R.id.Miss2_01, R.id.Miss2_02, R.id.Miss2_03,
        R.id.Miss2_04, R.id.Miss2_05, R.id.Miss2_06, R.id.Miss2_07,
        R.id.Miss2_08, R.id.Miss2_09, R.id.Miss2_10 )
    
    // カードのザンギ番号用配列
    var nakami = arrayOf(-1,-1,-1,-1,-1,-1,-1,-1)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        findViewById<TextView>(R.id.Combo2).text = combo.toString()
        findViewById<TextView>(R.id.Score2).text = score.toString()
        findViewById<TextView>(R.id.HighScore2).text = highscore.toString()
        ...
        findViewById<ImageButton>(R.id.PlainBase2).setOnClickListener(baseListener)

        // ゲームエンドバナーへのイベントリスナの紐づけ
        val gameendListener = GameEnd()
        findViewById<ImageButton>(R.id.GameEnd2).setOnClickListener(gameendListener)
    }
    ...
    private inner class CardTap : View.OnClickListener {
        override fun onClick(v: View){
            ...
            if(status == 0) {        // 未判定状態だったらカードをオープンする
                ...
                if(cardopen >= cardnum) {
                    mp.stop()
                    findViewById<ImageButton>(R.id.GameEnd2).setImageResource(R.drawable.clear_stage2)
                    findViewById<ImageButton>(R.id.GameEnd2).visibility = View.VISIBLE
                    soundPool.play(omedetou, 1.0f, 1.0f, 0, 0, 1.0f)
                }
                // ミスが１０回になったときの処理
                if(miss >= 10) {
                    mp.stop()
                    findViewById<ImageButton>(R.id.GameEnd2).setImageResource(R.drawable.gameover)
                    findViewById<ImageButton>(R.id.GameEnd2).visibility = View.VISIBLE
